...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!backendRes.ok) {
      ...
      loginUrl.searchParams.set(
        'error',
        errData?.message || 'Could not complete LinkedIn sign-in. Please try again.',
      );
      return NextResponse.redirect(loginUrl);
    }
    ...
    if (!jwtToken) {
      const loginUrl = new URL('/login', request.url);
      loginUrl.searchParams.set('error', 'LinkedIn sign-in did not return a valid session. Please try again.');
      return NextResponse.redirect(loginUrl);
    }
